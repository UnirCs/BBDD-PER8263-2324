...
public class OracleApplication {
    ...
    public static void main(String[] args) {
        ...
        try(Connection connection = new OracleDatabaseConnector("localhost", SERIVCE_NAME).getConnection()) {
            ...
            log.info("Se inserta un country en oracle");
            OracleCountry spain = new OracleCountry("ES", 1, "Spain");
            upsert(connection, spain);

            log.info("Se inserta un job en oracle");
            OracleJob fullStack = new OracleJob("IT_F.TACK", "Full Stack", 6600, 8350);
            upsert(connection, fullStack);
        } catch (Exception e) {
